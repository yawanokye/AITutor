(() => {
  ...
  function setAuthMode(mode) {
    ...
    $('authTitle').textContent = bootstrap ? 'Create first administrator' : registering ? 'Create student account' : 'Sign in';
    $('submitAuth').textContent = bootstrap ? 'Create administrator' : registering ? 'Create student account' : 'Sign in';
    ...
    $('authHelp').textContent = bootstrap
      ...
      : registering
        ? 'Student accounts may enrol in courses using codes generated by lecturers.'
        : 'Administrators and lecturers sign in with accounts created for them. Students may also sign in here.';
  ...
  async function submitAuth(event) {
    ...
    try {
      ...
      $('authForm').reset();
      setAuthMode('login');
      if (state.user.must_change_password) await loadDashboard();
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    setAuthMode('login'); await loadConfig(); await restoreUser();
  });
